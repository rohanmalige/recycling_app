...
class Detector {
  ...
  Future<String> classifyObject(String label) async {
    try {
          ...
          'messages': [
            {
              'role': 'user',
              'content':
                  'start with Yes or No and keep it brief and tell if the item is recyclable, keep it very short, classify which recycling bin it belongs to paper, plastic, glass, ewaste,metal,organic: $label?'
            }
          ],
          'max_tokens': 10,
        }),
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
            'Failed to classify object: ${response.statusCode}, ${response.body}');
        throw Exception('Failed to classify object');
      }
    } catch (e) {
      debugPrint('Exception during classification: $e');
      throw Exception('Failed to classify object');
    }
  }
  ...
  void _handleCommand(_Command command) async {
    switch (command.code) {
      case _Codes.init:
      ...
      case _Codes.result:
        _isReady = true;
        final results = command.args?[0] as Map<String, dynamic>;
        final List<Recognition> recognitions = results['recognitions'];
        int recyclableCount = 0;
        for (var recognition in recognitions) {
          try {
            final classification = await classifyObject(recognition.label);
            recognition.classification = classification;
            if (classification.toLowerCase().contains('recyclable')) {
              recognition.incrementRecyclableCount();
              recyclableCount++;
            }
          } catch (e) {
            recognition.classification = 'Error';
            debugPrint('Error classifying object: $e');
          }
        }
        results['recyclableCount'] =
            recyclableCount; // Add recyclable count to results
        resultsStream.add(results);
        break;
      case _Codes.ready:
        _isReady = true;
        break;
